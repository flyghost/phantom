...
Wave::Wave(Ui::PhantomClass *m_ui)
{
	//
	// Set up the GUI
	//
...
//信号槽初始化
void Wave::tcpConnectInit()
{
	QObject::connect(ui->waveTcpClientConnectButton, &QPushButton::clicked, this, &Wave::on_waveTcpClientConnectButton_clicked);
	connect(tcpSocket, &QTcpSocket::readyRead, this, &Wave::readMessageFromServer);
	connect(tcpSocket, SIGNAL(error(QAbstractSocket::SocketError)), this, SLOT(connectError(QAbstractSocket::SocketError)));

}

//连接按钮事件
void Wave::on_waveTcpClientConnectButton_clicked()
{
	if ((ui->waveTcpClientConnectButton->text()) == QString::fromLocal8Bit("连接"))
	{
		host = ui->waveTcpClientHostLineEdit->text();
		port = ui->waveTcpClientPortLineEdit->text();

		qDebug() << "host:" << host << "port:" << port;

		tcpSocket->connectToHost(host, port.toInt());

		ui->waveTcpClientConnectButton->setText(QString::fromLocal8Bit("断开"));
	...
	else
	{
		...
		ui->waveTcpClientConnectButton->setText(QString::fromLocal8Bit("连接"));
		//qDebug() << "disconnect";
	}
}

//读取消息
void Wave::readMessageFromServer()
{
	recvMesgByte = tcpSocket->readAll();

	recvMesgLength += recvMesgByte.length();

	ui->waveTcpClientReceiveBrowser->append(QString(recvMesgByte));

	qDebug() << "recv len:" << recvMesgLength;

	if (recvMesgLength > 20000)
	{
		ui->waveTcpClientReceiveBrowser->clear();
		recvMesgLength = 0;
	}

	QStringList list = QString(recvMesgByte).split("#");
	...
	if(list.count()>=4)
		onDataTimer(list.at(1).toFloat(), list.at(2).toFloat(), list.at(3).toFloat());
...
//连接错误
void Wave::connectError(QAbstractSocket::SocketError)
{
...
void Wave::waveInit()
{
	...
	m_HScrollBar = new QScrollBar(Qt::Horizontal, waveWidget);
...
//
void Wave::onMouseUsageChanged(int mouseUsage)
{
...
//
void Wave::onSave(bool)
{
	QString fileName = QFileDialog::getSaveFileName(waveWidget, "Save", "chartdirector_demo",
...
//
void Wave::onUpdatePeriodChanged(QString text)
{
...
//
void Wave::onDataTimer(double dataA, double dataB, double dataC)
{
	// The current time
...
//
void Wave::onHScrollBarChanged(int value)
{
...
//
void Wave::updateControls(QChartViewer *viewer)
{
...
// Draw chart
//
void Wave::drawChart(QChartViewer *viewer)
{
	...
	c->addTitle("Phantom Height Wave", "arial.ttf", 18);
	...
	c->yAxis()->setTitle("Realtime Height (HPA)", "arialbd.ttf", 12);
	...
	layer->addDataSet(viewPortDataSeriesA, 0xff0000, "Data1");
	layer->addDataSet(viewPortDataSeriesB, 0x00cc00, "Data2");
	layer->addDataSet(viewPortDataSeriesC, 0x0000ff, "Data3");
...
//
void Wave::onMouseMovePlotArea(QMouseEvent *)
{
...
//
void Wave::trackLineLabel(XYChart *c, int mouseX)
{
